...
TASK_TUTOR_PAYOUT = "tutor_payout"
...
def enqueue_payment_task(task: str, payload: dict[str, Any] | None = None):
    ...
    if backend in {"sync", "inline"}:
        return process_payment_task(message["task"], message["payload"])
    if backend == "rq":
...
def _queue():
    ...
    return Queue(
        str(getattr(settings, "PAYMENT_QUEUE_NAME", "prepvilla-payments")),
        connection=redis_connection(),
        default_timeout=int(getattr(settings, "PAYMENT_QUEUE_JOB_TIMEOUT_SECONDS", 300)),
        default_result_ttl=int(getattr(settings, "PAYMENT_QUEUE_RESULT_TTL_SECONDS", 3600)),
        default_failure_ttl=int(getattr(settings, "PAYMENT_QUEUE_FAILURE_TTL_SECONDS", 86400)),
    )
...
def run_rq_worker(*, burst: bool = False) -> None:
    ...
    connection = redis_connection()
    worker = Worker([_queue()], connection=connection)
    worker.work(
        burst=burst,
...
def process_payment_task(task: str, payload: dict[str, Any] | None = None):
    ...
    if task == TASK_FLUTTERWAVE_WEBHOOK:
        from .payments import process_flutterwave_webhook

        return process_flutterwave_webhook(payload.get("body") or payload)

    if task == TASK_TUTOR_PAYOUT:
        from .payments import process_tutor_payout

        payout_id = str(payload.get("payout_id") or "").strip()
        if not payout_id:
            raise PaymentQueueError("tutor_payout requires payout_id")
        return process_tutor_payout(payout_id)

    if task == TASK_PROCESS_BOOKING_PAYOUTS:
        from .models import Booking
        from .payments import process_booking_payout
        ...
        return process_booking_payout(booking)
    ...
    if task == TASK_PROCESS_SUBSCRIPTION_RENEWALS:
        from .payments import process_due_subscription_renewals

        return {"status": "ok", "result": process_due_subscription_renewals()}

    if task == TASK_RECONCILE_PENDING_PAYMENTS:
        from .payments import backfill_payment_accounting_records, reconcile_pending_customer_payments
