...
const verificationSelectTextStyle = { fontSize: "14px" } as const;
...
export default function DashboardVerificationPage({ accountRole = "tutor" }: { accountRole?: "student" | "tutor" }) {
  ...
  const initialFormLoadStartedRef = useRef(false);
  ...
  useEffect(() => {
    if (isApproved) clearDraft();
  }, [clearDraft, isApproved]);
  ...
  }, [isApproved, isStudent, me?.id, router]);
  ...
  const hasCompleteVerificationInfo = Boolean(
    (!isStudent || (homeState.trim() && homeCity.trim() && homeAddress.trim())) &&
    (isStudent || selectedQualifications.length > 0) &&
    ninNumber.trim() &&
    !ninNumberError &&
    ...
    !mobileNumberError &&
    hasIdDocument &&
    (isStudent || hasQualificationDocument),
  );
  ...
  async function uploadFile(file: File, kind: "id_document" | "qualification_document") {
  ...
  const load = useCallback(async () => {
    const shouldHydrateForm = !initialFormLoadStartedRef.current;
    if (shouldHydrateForm) initialFormLoadStartedRef.current = true;
    ...
    if (resVer.ok) {
      ...
      if (shouldHydrateForm) {
        ...
        setNote(resVer.data.notes || "");
      }
    }
    if (shouldHydrateForm) setIsFormReady(true);
  ...
  async function submit() {
    ...
    try {
      ...
      if (!nextDocumentUrls[0] || (!isStudent && !nextDocumentUrls[1])) {
        throw new Error(isStudent
          ? "Please upload your ID document."
          : "Please upload both your ID document and qualification document.");
      }
    } catch (e) {
    ...
    const res = await api.post<VerificationResponse>(verificationEndpoint, {
      ...(isStudent ? { homeState, homeCity, homeAddress } : {}),
      ...(!isStudent ? { qualification: selectedQualifications.join(", ") } : {}),
      firstName,
      middleName,
      lastName,
      email,
    ...
    setNotification({
      type: "success",
      message: isStudent
        ? "Verification completed successfully. Continue to your profile to provide the remaining information."
  ...
  return (
    <RequireAuth allow={[accountRole]}>
      <div className="grid gap-4 [&_input]:text-[14px] [&_option]:!text-[14px] [&_select]:!text-[14px]">
        <div className="form-panel rounded-xl p-4">
        ...
        <div className="form-panel rounded-xl p-4">
          {/* Status Banner */}
          <div className="mb-1 flex flex-wrap items-center justify-between gap-2 border-b border-border pb-4">
            <div>
        ...
        <div className="form-panel rounded-xl p-4 [&_label>div]:text-[13px] [&_label>div]:text-black/55 [&_label]:gap-0.5 [&_label]:mb-0.5 [&_label]:text-[13px] [&_label]:text-black/55 [&_select]:h-11">
          ...
          <p className="mb-4 text-xs leading-[20px] text-black/55">
            Enter your personal details exactly as they appear on your NIN or BVN records
          </p>

          <div className={`text-[16px] grid gap-6 ${isApproved ? "opacity-75" : ""}`}>
            ...
            <div className="grid gap-x-2 gap-y-6 md:grid-cols-3">
              ...
              <Input
                ...
                disabled={isApproved}
                placeholder="start with +234 followed by 10 digits."
            ...
            <div className="grid gap-x-2 gap-y-6 text-[16px] md:grid-cols-4">
              <Select
                label="Country of Birth *"
                style={verificationSelectTextStyle}
              ...
              {isNigeriaCountryOfBirth ? (
                ...
                <Input
                  ...
                  onChange={(e) => setLgaOfOrigin(e.target.value)}
                  disabled={isApproved}
                  placeholder="enter your local government of region"
                  required
                />
              )}
            </div>

            <div className={`grid gap-x-2 gap-y-6 ${isStudent ? "md:grid-cols-2" : "md:grid-cols-3"}`}>
              <Input
                label="NIN Number *"
                value={ninNumber}
              ...
              <Input
                ...
                disabled={isApproved}
                inputMode="numeric"
                placeholder={isStudent ? "(Optional) must be 11 digits.." : "must be 11 digits."}
                // helperText={isStudent ? "Optional. BVN must be exactly 11 digits.." : "BVN must be exactly 11 digits."}
              ...
              {!isStudent ? <div>
                <Select
                  ...
                  labelClassName="!text-[13px] !leading-5"
                  className="h-11"
                  style={verificationSelectTextStyle}
        ...
        {isStudent ? <div>
          <div className="form-inset rounded-xl p-4 [&_label>div]:text-[13px] [&_label>div]:text-black/55 [&_label]:gap-0.5 [&_label]:mb-0.5 [&_label]:text-[13px] [&_label]:text-black/55 [&_select]:h-11">
            <div className="mb-4">
        ...
        <div>
          <div>
            <div className={`grid gap-x-2 gap-y-6 ${isStudent ? "md:grid-cols-1" : "md:grid-cols-2"}`}>
              <div className="form-inset grid gap-0.5 rounded-xl p-3">
                <span className="text-[13px] leading-[20px] text-black/55">
                ...
                <span className="mt-1.5 min-h-5 text-xs text-black/55">
                  {idDocument ? idDocument.name : state?.documentUrls?.[0] ? "Saved ID document on file" : "No file selected"}
                </span>
              </div>
              {!isStudent ? <div className="form-inset grid gap-0.5 rounded-xl p-3">
                <span className="text-[13px] leading-[20px] text-black/55">
          ...
          <div className="mt-6">
            ...
            {isApproved && (
              <div className="mt-4 p-3 bg-green-50 border border-green-200 rounded-lg text-green-700 text-sm">
                {isStudent
                  ? "✓ Verification Successful - Complete your profile and contact tutors or book lessons"
                  : "✓ Verification Successful - Complete your profile and beome visible to students"}
              </div>
            )}
          </div>
